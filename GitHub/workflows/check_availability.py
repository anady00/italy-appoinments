...
def dismiss_cookie_banner(page):
    """Best-effort dismissal of any cookie-consent overlay before we try to
    click anything else. Never raises -- if there's no banner this is a no-op."""
    candidates = [
        "text=Conferma", "text=Accetta", "text=Accept", "text=Accept all",
        "text=I agree", "button:has-text('Conferma')", "#onetrust-accept-btn-handler",
    ]
...
def login(page):
    ...
    # Force Italian so all our text-based fallback selectors stay valid
    # regardless of the runner's default Accept-Language.
    page.goto(f"{BASE_URL}/Home", wait_until="domcontentloaded")
    dismiss_cookie_banner(page)
    debug_dump(page, "01_home")

    # Prefer the language-independent way of finding the login link: it always
    # points at the iam.esteri.it OAuth authorize endpoint, regardless of UI language.
    login_link = page.locator("a[href*='iam.esteri.it']").first
    text_fallbacks = [
        "text=Effettuare il Login", "text=Login", "text=Accedi", "text=Sign in",
    ]
    ...
    try:
        ...
        print(f"[login] href-based click failed: {e}")

    if not clicked:
        for sel in text_fallbacks:
            try:
    ...
    if not clicked:
        debug_dump(page, "01b_no_login_link")
        raise RuntimeError(
            "Could not find any login link/button on the home page. "
            "Re-run with DEBUG=1 and inspect debug/01_home.html / debug/01b_no_login_link.png"
        )
